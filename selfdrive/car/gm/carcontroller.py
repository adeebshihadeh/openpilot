...
class CarController():
  ...
  def update(self, enabled, CS, frame, actuators, \
    ...
    if (frame % self.P.STEER_STEP) == 0:
      lkas_enabled = enabled and not CS.steer_not_allowed and CS.out.vEgo > self.P.MIN_STEER_SPEED
      if lkas_enabled:
    ...
    if self.car_fingerprint not in SUPERCRUISE_CARS:
      ...
      if not enabled:
        ...
        apply_gas = self.P.MAX_ACC_REGEN
        apply_brake = 0
      else:
        apply_gas = int(round(interp(final_pedal, self.P.GAS_LOOKUP_BP, self.P.GAS_LOOKUP_V)))
        apply_brake = int(round(interp(final_pedal, self.P.BRAKE_LOOKUP_BP, self.P.BRAKE_LOOKUP_V)))
      ...
      if (frame % 4) == 0:
        ...
        near_stop = enabled and (CS.out.vEgo < self.P.NEAR_STOP_BRAKE_PHASE)
      ...
      if frame % self.P.ADAS_KEEPALIVE_STEP == 0:
      ...
      if frame % self.P.CAMERA_KEEPALIVE_STEP == 0 \
          or lka_icon_status != self.lka_icon_status_last:
        can_sends.append(gmcan.create_lka_icon_command(CanBus.SW_GMLAN, lka_active, lka_critical, alert_out))
